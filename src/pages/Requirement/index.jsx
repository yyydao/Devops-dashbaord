...
class Requirement extends Component {
  ...
  showConfirm = (demandID) => {
    confirm({
      ...
      onOk: () => {
        this.deleteRequirement(demandID)
      }
    });
  }
  popoverContent = (statuses) => {
    if (statuses) {
      return <div>{statuses.map((item, index) => <p key={index} className="status-container">
        <span>{item.status}</span><span>{item.size}个</span></p>)}</div>
    } else {
      return <div>暂无状态</div>
    }
  }
      ...
      <div>
        ...
        <div className="content-container">
          <Card
            ...
            extra={<Button type="primary" icon="plus" onClick={() => {
        ...
        <Modal
          ...
          <Spin spinning={searchLoading}>
            <Row>
              <Col span={20}><Input placeholder='TAPD_ID（填入Tapd关联的父需求id）' value={searchTapdId} onChange={(e) => {
                this.setState({searchTapdId: e.target.value.trim()})
              }}/></Col>
              <Col span={4}><Button type="primary" style={{cssFloat: "right"}}
                                    onClick={() => this.getRequirement()}>刷新</Button></Col>
            </Row>
            ...
            <p style={{
              marginTop: 24,
              marginBottom: 0
            }}>预计时间： {searchRequirement.begin || '—'}{searchRequirement.begin && searchRequirement.due ? '~' : '/'}{searchRequirement.due || '—'}</p>
          </Spin>
        </Modal>
      </div>
    )
